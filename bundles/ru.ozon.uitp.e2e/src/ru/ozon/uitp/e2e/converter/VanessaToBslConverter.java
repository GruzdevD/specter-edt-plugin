...
public class VanessaToBslConverter {
	...
	public String generateBslModuleCode(VanessaScenario scenario) {
		...
		sb.append("\t\t\"Имя, Описание, Теги, Отключен, ПричинаОтключения\",\n");
		...
		if (scenario.isSkipped()) {
			...
			sb.append("\t\t\t\"").append(testMethodName).append("\", СП_Тестирование.СтатусПропущен(), \"").append(escapeBslString(scenario.getSkipReason())).append("\", 0, Новый Массив, \"\"));\n");
		} else {
